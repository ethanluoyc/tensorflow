...
def if_mkl_ml(if_true, if_false = []):
    ...
    """
    return select({
        clean_dep("//xla/tsl/mkl:build_with_mkl_opensource"): if_false,
        clean_dep("//xla/tsl/mkl:build_with_mkl"): if_true,
        clean_dep("//conditions:default"): if_false,
    })
...
def if_mkl_lnx_x64(if_true, if_false = []):
    ...
    """
    return select({
        clean_dep("//xla/tsl/mkl:build_with_mkl_lnx_x64"): if_true,
        "//conditions:default": if_false,
    })
...
def if_enable_mkl(if_true, if_false = []):
    ...
    """
    return select({
        clean_dep("//xla/tsl/mkl:enable_mkl"): if_true,
        "//conditions:default": if_false,
    })
...
def mkl_deps():
    ...
    """
    return select({
        clean_dep("@xla//xla/tsl/mkl:build_with_mkl_aarch64"): ["@mkl_dnn_acl_compatible//:mkl_dnn_acl"],
        clean_dep("@xla//xla/tsl:linux_x86_64_with_onednn_async"): ["@onednn_async//:mkl_dnn"],
        clean_dep("@xla//xla/tsl:linux_x86_64"): ["@onednn//:mkl_dnn"],
        clean_dep("@xla//xla/tsl:windows"): ["@onednn//:mkl_dnn"],
        "//conditions:default": [],
    })
...
def if_onednn_async(if_true, if_false = []):
    ...
    """
    return select({
        clean_dep("//xla/tsl:linux_x86_64_with_onednn_async"): if_true,
        "//conditions:default": if_false,
    })
...
def onednn_v3_define():
    ...
    """
    return select({
        clean_dep("//xla/tsl/mkl:build_with_mkl_aarch64"): ["-DENABLE_ONEDNN_V3"],
        clean_dep("//xla/tsl:linux_x86_64"): ["-DENABLE_ONEDNN_V3"],
        clean_dep("//xla/tsl:windows"): ["-DENABLE_ONEDNN_V3"],
        "//conditions:default": [],
    })
...
def if_mkldnn_openmp(if_true, if_false = []):
    ...
    """
    return select({
        clean_dep("//xla/tsl/mkl:build_with_mkldnn_openmp"): if_true,
        "//conditions:default": if_false,
    })

def if_mkldnn_aarch64_acl(if_true, if_false = []):
    return select({
        clean_dep("//xla/tsl/mkl:build_with_mkl_aarch64"): if_true,
        "//conditions:default": if_false,
    })
...
def if_graph_api(if_true, if_false = []):
    ...
    return select({
        clean_dep("//xla/tsl:linux_x86_64"): if_true,
        clean_dep("//xla/tsl/mkl:build_with_mkl_aarch64"): if_true,
        "//conditions:default": if_false,
    })
